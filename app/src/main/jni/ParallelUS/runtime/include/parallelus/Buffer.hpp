...
#ifndef parallelus_BUFFER_HPP
#define parallelus_BUFFER_HPP
...
public:
     ...
     */
    void copyToJArray(JNIEnv *env, jarray array, int lastOutput);

    /**
     * Helper method to copy size() bytes to an Android bitmap. Throws
     * EmptyBufferError if the buffer doesn't have any data.
     */
    void copyToAndroidBitmap(JNIEnv *env, jobject bitmap, int lastOutput);
    void ExternCopyToAndroidBitmap(JNIEnv *env, jobject bitmap, jobject lock);

    /**
     * Copies size() bytes to an host pointer. Throws
     * EmptyBufferError if the buffer doesn't have any data.
     */
    void copyTo(void *host, int lastOutput);
...
private:
	...
	void setSocketNumber(int socketfd);
    ...
    void * _genericPtr;
    int _externBuffer; 					/// 1=Buffer changed by networker
    int _id;
