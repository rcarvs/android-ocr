...
#ifndef parallelus_TASK_HPP
#define parallelus_TASK_HPP
...
public:
     ...
     * task.
     */
    typedef std::function<void (DevicePtr &, KernelHash &, unsigned type)> KernelFunction;
...
private:
	...
	void createExternKernels(JNIEnv *env);
	
	void sendKernelsName(int socketfd);
	
    /// Calls the configFunction currently set or does nothing if it wasn't set.
    // type: variable responsible to indicate if this task will be executed by a inter worker (type==1)
    //       or if the task will be executed by a networker (type==0) [TODO] PENSAR em controle melhor.
    inline void callConfigFunction(DevicePtr device, unsigned type) {
        if(_configFunction){
            _configFunction(device, _kernelHash, type);
        }
    }

    /// Calls the finishFunction currently set or does nothing if it wasn't set.
    inline void callFinishFunction(DevicePtr device, unsigned type) {
        if(_finishFunction)
            _finishFunction(device, _kernelHash, type);
    }
